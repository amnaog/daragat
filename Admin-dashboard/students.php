...
if (isset($_POST['action']) && $_POST['action'] === 'add') {
    ...
    mysqli_query($conn, "INSERT INTO students (full_name, email, phone, level) VALUES ('$name', '$email', '$phone', '$level')");
...
if (isset($_POST['action']) && $_POST['action'] === 'update') {
    ...
    $phone = mysqli_real_escape_string($conn, $_POST['phone']);
    $level = mysqli_real_escape_string($conn, $_POST['level']);
    mysqli_query($conn, "UPDATE students SET full_name='$name', email='$email', phone='$phone', level='$level' WHERE id=$id");
    header("Location: students.php");
    exit();
}
...
<div class="main">
    <div class="card">
        <h2><?php echo $editMode ? 'Edit' : 'Add'; ?> Student</h2>
        <form method="POST">
            ...
            <input type="text" name="level" placeholder="Level" required value="<?php echo $editMode ? $editData['level'] : ''; ?>">
            <button type="submit"><?php echo $editMode ? 'Update' : 'Add'; ?> Student</button>
        </form>
    </div>
            ...
            <?php while ($row = mysqli_fetch_assoc($students)): ?>
                <tr>
                    ...
                    <td>
                        ...
                        <a class="delete" href="?delete=<?php echo $row['id']; ?>" onclick="return confirm('Delete this student?');">Delete</a>
                    </td>
                </tr>
            <?php endwhile; ?>
        </table>
